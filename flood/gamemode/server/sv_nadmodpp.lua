...
if not NADMOD then
  ...
  NADMOD = util.JSONToTable(file.Read("nadmod_config.txt","DATA") or "") or
    {Users = {}, Groups = {}, Bans = {}, PPConfig = {}}
  function NADMOD.Save()
    file.Write(
      "nadmod_config.txt",
      util.TableToJSON({Users = NADMOD.Users, Groups = NADMOD.Groups,
			Bans = NADMOD.Bans, PPConfig = NADMOD.PPConfig})
    )
  end
...
if not NADMOD.Props then
  ...
  -- {entid = {Ent = ent, Owner = ply, SteamID = ply:SteamID(), Name = ply:Nick() or "W" or "O"}}
  NADMOD.Props = {}
  -- A smaller buffer of PropOwner names to send to current players
  ...
  timer.Create(
    "WarnOtherPPs", 0, 1,
    function()
      -- Print a harmless error if we detect other PP's active, since this often leads to confusion.
      -- NPP will still load fine
...
-- If you admin mod doesn't seem to work with NADMOD PP, Try overriding this function.
...
function NADMOD.PlayerCanTouch(ply, ent)
  ...
  if not tobool(NADMOD.PPConfig["toggle"]) or ent:IsWorld() then
    return true
  elseif !IsValid(ent) or !IsValid(ply) or ent:IsPlayer() or !ply:IsPlayer() then
    return false
  end
...
function NADMOD.PlayerCanTouchSafe(ply, ent)
  if !IsValid(ent) or ent:IsPlayer() then
    return
  elseif !NADMOD.PlayerCanTouch(ply,ent) then
    return false
  end
end
...
function NADMOD.OnPhysgunReload(weapon, ply)
  ...
  if not tr.HitNonWorld or !tr.Entity:IsValid() or tr.Entity:IsPlayer() then
    return
  elseif !NADMOD.PlayerCanTouch(ply, tr.Entity) then
    return false
  end
end
...
function NADMOD.GravGunPickup(ply, ent)
  if !IsValid(ent) or ent:IsPlayer() then
    return
  elseif NADMOD.Props[ent:EntIndex()] and NADMOD.Props[ent:EntIndex()].Name == "W" then
    return
  elseif !NADMOD.PlayerCanTouch(ply,ent) then
    return false
  end
...
function NADMOD.CanTool(ply, tr, mode)
  ...
  if !NADMOD.PlayerCanTouch(ply, ent) then
    if not ((NADMOD.Props[ent:EntIndex()] or {}).Name == "W" and
	(mode == "wire_debugger" or mode == "wire_adv"))
    then
      return false
    end
...
function NADMOD.PlayerUse(ply, ent)
  if !NADMOD.PPConfig["use"] or NADMOD.PlayerCanTouch(ply, ent) or
    (ent:IsValid() and NADMOD.Props[ent:EntIndex()].Name == "W")
  then
    return
  else
    return false
...
function NADMOD.EntityRemoved(ent)
  ...
  if ent:IsValid() and ent:IsPlayer() and not ent:IsBot() then
    ...
    if NADMOD.PPConfig.autocdp > 0 and
      (NADMOD.PPConfig.autocdpadmins or not NADMOD.IsPPAdmin(ent))
    then
      timer.Create(
	"NADMOD.AutoCDP_" .. steamid, NADMOD.PPConfig.autocdp, 1,
	function()
	  local count = NADMOD.CleanupPlayerProps(steamid)
	  if count > 0 then NADMOD.Notify(nick .. "'s props (" .. count .. ") have been autocleaned.")
	  end
	end
      )
...
hook.Add(
  "PlayerConnect", "NADMOD.ClearAutoCDP",
  function(nick, address) -- This occurs early but is unreliable
...
function NADMOD.DebugTotals(ply, cmd, args)
  ...
  if IsValid(ply) then -- PrintTable only shows in serverconsole
    return
  end
...
net.Receive(
  ...
  function(len,ply)
    ...
    for steamid, bool in pairs(net.ReadTable()) do
      if players[steamid] and
	(not bool or not (NADMOD.IsPPAdmin(players[steamid]) and
			    NADMOD.PPConfig["adminall"]))
