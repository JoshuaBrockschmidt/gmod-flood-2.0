...
function GM:HUDDrawScoreBoard()
  ...
  -- Header
  draw.RoundedBoxEx(6, xOffset, yOffset, boardWidth, self.ScoreDesign.HeaderY,
		    Color(24, 24, 24, 255), true, true, false, false)
  draw.RoundedBoxEx(6, xOffset, yOffset + 30, boardWidth, self.ScoreDesign.Height - 25,
		    Color(240, 240, 240, 255), false, false, true, true)
  -- Header text
  local ySpacing = yOffset + 25
  draw.SimpleText(GetGlobalString("ServerName"), ScoreboardFont,
		  xOffset + boardWidth * 0.5, ySpacing - 10,
  ...
  draw.SimpleText("Cash", ScoreboardFont, xOffset + boardWidth - (colWidth*3) + 8, ySpacing,
		  color_black)
  draw.SimpleText("Deaths", ScoreboardFont, xOffset + boardWidth - (colWidth*2) + 8, ySpacing,
		  color_black)
  draw.SimpleText("Ping", ScoreboardFont, xOffset + boardWidth - (colWidth*1) + 8, ySpacing,
		  color_black)
  ...
  for team,info in pairs(ScoreboardInfo) do
    ...
    draw.RoundedBox(0, xOffset + 5, yPosition, boardWidth - 10, 19,
		    Color(info.Color.r, info.Color.g, info.Color.b, 255))

    yPosition = yPosition + 2

    draw.SimpleText(teamText, ScoreboardFont, xOffset + boardWidth * 0.5, yPosition + 8,
		    color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	draw.RoundedBox(0, xOffset + 10, yPosition, boardWidth - 20, 16, Color(0, 0, 0, 100))
      ...
      local px = xOffset + 16
      draw.SimpleText(plinfo.Name, ScoreboardFont, px, yPosition, Color(24, 24, 24, 255),
		      TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)

      px = xOffset + boardWidth - (colWidth*3) + 8
      draw.SimpleText("$" .. plinfo.cash, ScoreboardFont, px, yPosition, Color(0, 100, 0, 255),
		      TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)

      px = xOffset + boardWidth - (colWidth*2) + 8
      draw.SimpleText(plinfo.Deaths, ScoreboardFont, px, yPosition, Color(100, 0, 0, 255),
		      TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)

      px = xOffset + boardWidth - (colWidth*1) + 8
      draw.SimpleText(plinfo.Ping, ScoreboardFont, px, yPosition, Color(0, 0, 100, 255),
		      TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
