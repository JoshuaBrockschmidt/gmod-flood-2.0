...
function PANEL:Init()
  ...
  if PropCategories then
    ...
    LocalPlayer():ChatPrint("Failed to load prop categories table - please notify an administrator.")
  end
  ...
  if Props then
	...
	surface.PlaySound("ui/buttonclick.wav")
      ...
      if v.Description and v.Health and v.Price then
	ItemIcon:SetTooltip(
	  string.format("%s\nHealth: %s\nPrice: $%s", v.Description, v.Health, v.Price)
	)
      else
	ItemIcon:SetTooltip("Failed to load tooltip - Missing Description")
      ...
      if v.Group then
	self.FloodPropIconList[v.Group]:AddItem(ItemIcon)
      end
      ItemIcon:InvalidateLayout(true)
    end
  else
    LocalPlayer():ChatPrint("Failed to load prop table - please notify an administrator.")
  end
end
