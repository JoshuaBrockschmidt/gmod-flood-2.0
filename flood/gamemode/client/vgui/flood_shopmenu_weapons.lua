...
function PANEL:Init()
  ...
  if WeaponCategories then
    ...
    LocalPlayer():ChatPrint("Failed to load weapon categories table - please notify an administrator.")
  end
  ...
  if Weapons then
	...
	ItemIcon:SetTooltip("Failed to load tooltip - Missing Description")
      ...
      if v.Group then
	self.FloodWeaponIconList[v.Group]:AddItem(ItemIcon)
      end
      ItemIcon:InvalidateLayout(true)
    end
  else
    LocalPlayer():ChatPrint("Failed to load weapon table - please notify an administrator.")
  end
end
